...
| AT-31 | Sign-in through the Deepline session | Mock Deepline that accepts a `better-auth.session_token` cookie; no destinations | Open the panel signed out; add the cookie; pick a play from the panel; push | Signed out: sign-in card and "Choose where to send". After the cookie: header shows the rep's email, "Pick the play" card, play list loads with `credentials: include` and no bearer; the run carries the cookie and no `Authorization`; the saved destination has an empty `apiKey`; the cookie value never appears in extension storage (and the extension has no `cookies` permission). | "sign-in flow: a Deepline session cookie…" |
| AT-32 | Page scripts cannot drive the extension | Webhook destination; Sales Navigator search fixture | From page JavaScript: dispatch `click()` on a row pill and on the dock's Push/Select-all buttons, post a forged `LWE_BRIDGE` message for a non-allow-listed URL; then click for real | Synthetic clicks select nothing and send nothing (webhook receives 0); the forged response is not ingested; a real click on the pill selects the row and a real Push delivers it. | "page scripts cannot drive the extension…" |
...
| Session auth | Session endpoint with `credentials: include` + `redirect: error`, no cookies API; user/org parsed; signed-out, 503 and network errors handled; identity key; Deepline-tab refresh; queued runs bound to the queuing identity and failed on sign-out / account switch; `signed_out` rejections; sign-in URL | `telemetry-auth.test.ts`, `worker.test.ts` |
