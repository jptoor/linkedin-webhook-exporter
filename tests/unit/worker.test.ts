...
describe("connection sync recovery", () => {
  it("reports an interrupted run without resuming LinkedIn reads", async () => {
    await boot({}, () => {
      fake.sessionStore.connectionSync = { status: "running", scanned: 40, queued: 40, skipped: 0, destination: "Hook", message: "Reading" };
    });
    expect(await send({ type: "CONNECTIONS_STATUS" }, PANEL)).toMatchObject({ status: "stopped", scanned: 40, queued: 40 });
